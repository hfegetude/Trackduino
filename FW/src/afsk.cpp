...
bool afsk_flush()
{
  //Serial.print("is fifo full? "); Serial.println(afsk_is_fifo_full_safe());
  while (! afsk_is_fifo_full_safe()) {
    //Serial.print("Packet pos: "); Serial.print(packet_pos); Serial.print(" Packet size: " ); Serial.println(afsk_packet_size);
...
AFSK_ISR_C
{
  if(is_more_than_half){
    if (afsk_is_fifo_empty()) {
      ...
      afsk_output_sample( (afsk_fifo_out()<8) | 0xFF);
    }
   
  }
  else{
    afsk_output_sample(0x00FF);
    is_more_than_half = 1;
  }
...
AFSK_ISR
{
  //Serial.print("Head: "); Serial.print(sample_fifo_head); Serial.print(" Tail: "); Serial.print(sample_fifo_tail); Serial.println(afsk_is_fifo_empty());
  if (afsk_is_fifo_empty()) {
    if (go) {
      sample_overruns++;
    }
