...
LOCAL_C_SRCS    ?= $(wildcard $(SRC_PATH)/*.c)
LOCAL_CPP_SRCS  ?= $(wildcard $(SRC_PATH)/*.cpp)
LOCAL_CC_SRCS   ?= $(wildcard $(SRC_PATH)/*.cc)
LOCAL_PDE_SRCS  ?= $(wildcard $(SRC_PATH)/*.pde)
LOCAL_INO_SRCS  ?= $(wildcard $(SRC_PATH)/*.ino)
LOCAL_AS_SRCS   ?= $(wildcard $(SRC_PATH)/*.S)
    ...
    ifeq ($(strip $(NO_CORE)),)
        ...
        ifneq ($(words $(LOCAL_PDE_SRCS) $(LOCAL_INO_SRCS)), 1)
            ifeq ($(words $(LOCAL_PDE_SRCS) $(LOCAL_INO_SRCS)), 0)
                ...
                $(error Need exactly one .pde or .ino file. This makefile doesn\'t support multiple .ino/.pde files yet $(LOCAL_SRCS))
            endif
        endif
